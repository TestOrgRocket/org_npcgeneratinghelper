...
public class SingleNPCGeneration : MonoBehaviour
{
    ...
    public void SaveNpc()
    {
        DataManager.CreateNPC(_currentNPC);
    ...
    public void GenerateNewNpc()
    {
        ...
        // Determine gender (random with 50/50 chance)
        ...
        newNpc.relationshipsIsSet = true;
        newNpc.relationshipStatus = Random.value > 0.5f;
        if (newNpc.relationshipStatus)
        {
            var partnerData = GenerateName(!isMale);
            newNpc.relationshipPartner = partnerData.firstName + " " + partnerData.lastName;
        }
        else
        {
            newNpc.relationshipPartner = "";
        }
        ...
        // Generate birthday (random date within last X years based on age)
